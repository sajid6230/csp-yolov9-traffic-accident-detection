...
class RepNCSPELAN4(nn.Module):
    # deeper csp-elan, but same input/output channels
    def __init__(self, c1, c2, c3, c4, c5=1):
        super().__init__()
        self.c = c3 // 2

        # 1x1 to get c3 channels
        self.cv1 = Conv(c1, c3, 1, 1)
        ...
        # 🔥 extra deeper branch
        self.cv4_extra = nn.Sequential(
            RepNCSP(c4, c4, c5),
            Conv(c4, c4, 3, 1)
        )

        # final fusion: c3 + 3*c4 → c2
        self.cv_out = Conv(c3 + 3 * c4, c2, 1, 1)

    def forward(self, x):
        y = list(self.cv1(x).chunk(2, 1))

        for m in (self.cv2, self.cv3, self.cv4_extra):
            y.append(m(y[-1]))
