...
class RepNCSPELAN4(nn.Module):
    # deeper csp-elan: add one more CSP branch
    def __init__(self, c1, c2, c3, c4, c5=1):  # ch_in, ch_out, mid1, mid2, repeat
        super().__init__()
        self.c = c3 // 2

        # 1) First 1x1 conv: expand channels and then split into two halves
        self.cv1 = Conv(c1, c3, 1, 1)

        # 2) First CSP-ELAN branch
        self.cv2 = nn.Sequential(
            RepNCSP(c3 // 2, c4, c5),
            Conv(c4, c4, 3, 1)
        )

        # 3) Second CSP-ELAN branch (as before)
        self.cv3 = nn.Sequential(
            RepNCSP(c4, c4, c5),
            Conv(c4, c4, 3, 1)
        )

        # 4) 🔥 New third CSP-ELAN branch (extra depth)
        self.cv4 = nn.Sequential(
            RepNCSP(c4, c4, c5),
            Conv(c4, c4, 3, 1)
        )

        # 5) Final 1x1 conv to fuse all branches
        #   Channels: c3/2 + c3/2 + c4 + c4 + c4 = c3 + 3*c4
        self.cv_out = Conv(c3 + 3 * c4, c2, 1, 1)

    def forward(self, x):
        # Split after first conv
        y = list(self.cv1(x).chunk(2, 1))  # [y0, y1], each c3//2 channels

        # Pass last branch through 3 CSP-ELAN sub-blocks sequentially
        for m in (self.cv2, self.cv3, self.cv4):
            y.append(m(y[-1]))

        # Concatenate all and fuse
        return self.cv_out(torch.cat(y, 1))

    def forward_split(self, x):
        # same idea but using split instead of chunk
        y = list(self.cv1(x).split((self.c, self.c), 1))
        for m in (self.cv2, self.cv3, self.cv4):
            y.append(m(y[-1]))
        return self.cv_out(torch.cat(y, 1))
...
# make sure Conv and RepNCSPELAN4 are already defined in this file

class CSPRepNCSPELAN4(nn.Module):
    """
    CSP-style wrapper around RepNCSPELAN4:
    - Align channels with 1x1 Conv
    - Split channels into two halves
    - One half goes through RepNCSPELAN4 (heavy path)
    - The other half is identity (shortcut path)
    - Concatenate and fuse with 1x1 Conv
    """
    def __init__(self, c1, c2, c3, n=3):
        super().__init__()
        # align input channels to c2
        self.cv_in = Conv(c1, c2, k=1, s=1)

        ch_half = c2 // 2

        # heavy path on half the channels
        self.heavy = RepNCSPELAN4(ch_half, ch_half, c3, n)

        # light path: just pass-through
        self.light = nn.Identity()

        # fuse both halves
        self.cv_out = Conv(c2, c2, k=1, s=1)

    def forward(self, x):
        x = self.cv_in(x)                 # [B, c2, H, W]
        x1, x2 = torch.chunk(x, 2, 1)     # split channels into 2 halves

        y1 = self.heavy(x1)               # processed half
        y2 = self.light(x2)               # shortcut half

        y = torch.cat((y1, y2), dim=1)    # CSP concat
        y = self.cv_out(y)                # mix features
        return y
